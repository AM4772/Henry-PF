...
const heroku = `https://pf-bookstore-main.onrender.com`;
                ...
                if (!response.enabled) {
                    if (response.dateSuspended) {
                        if (
                            !(
                                parseInt(today) <
                                parseInt(response.dateSuspended) + 86400000
                            )
                        ) {
...
export function asyncAutoLogin(token) {
    return async function (dispatch) {
        try {
            const response = (await axios.post('/login/autoLogin', { token }))
                .data;
            var today = Date.now();
            dispatch(loginUser(response));
            if (!response.enabled) {
...
export function asyncLoginAuth0(body) {
    return async function (dispatch) {
        try {
            const response = (await axios.post('/users/auth0/login', body))
                .data;
            localStorage.setItem('ALTKN', response.data.token);
            dispatch(loginUser(response.data));
        } catch (error) {
...
export function asyncDisableUser(ID) {
    return async function (dispatch) {
        try {
            const response = (await axios.put(`/users/${ID}?suspended=true`))
                .data;
            dispatch(getUserDetail(response));
        } catch (error) {
            console.error(error);
...
export function asyncEnableUser(ID) {
    return async function (dispatch) {
        try {
            const response = (await axios.put(`/users/${ID}?enabled=true`))
                .data;
            dispatch(getUserDetail(response));
            // satisfaction.fire({
            //   icon: "error",
